        ...
        @if(isset($wifi['data']['radios']) && count($wifi['data']['radios']) > 0)
            ...
            <div class="list-group-item bg-transparent border-secondary py-3 text-white">
                <div class="row align-items-center">
                    <div class="col-12 col-md-auto text-center mb-3 mb-md-0 d-flex justify-content-center align-items-center" style="width: 80px;">
                        <span class="badge border border-secondary p-2 d-inline-block text-center shadow-sm" style="background: rgba(255,255,255,0.05); min-width: 60px;">
                            <i class="bi bi-broadcast text-info" style="font-size: 1.2rem;"></i><br>
                            <small class="text-muted fw-normal" style="font-size: 0.75rem;">{{ $radio['id'] ?? 'radio0' }}</small>
                        </span>
                    </div>
                    <div class="col-12 col-md mb-3 mb-md-0 text-center text-md-start">
                        ...
                        <div class="text-soft small fw-bold">Canal: <span class="fw-normal">{{ $radio['channel'] ?? '?' }} (2.462 GHz)</span> <span class="text-muted mx-1">|</span> Tasa de bits: <span class="fw-normal">? Mbit/s</span></div>
                    </div>
                    <div class="col-12 col-lg-auto d-flex gap-2 justify-content-center">
                        <form action="{{ route('network.wifi.restart') }}" method="POST" class="m-0 p-0">
                            @csrf
                            <button type="submit" class="btn btn-secondary btn-sm fw-bold text-dark px-3" style="background: #e0e0e0; font-size:0.75rem;" onclick="return confirm('¿Seguro que deseas reiniciar la red Wi-Fi? Perderás la conexión momentáneamente.')">REINICIAR</button>
                        </form>
                        <button type="button" class="btn btn-sm text-white fw-bold px-3" style="background: #3eb2cc; font-size:0.75rem;" id="btnScanWifi">ESCANEAR</button>
                        <button type="button" class="btn btn-primary btn-sm fw-bold px-3" style="background: #397cbd; font-size:0.75rem;" data-bs-toggle="modal" data-bs-target="#addWifiModal" data-device="{{ $radio['id'] ?? 'radio0' }}">AÑADIR</button>
                    </div>
                </div>
            </div>
            ...
            @if(isset($wifi['data']['interfaces']))
                @foreach($wifi['data']['interfaces'] as $interface)
                    ...
                    <div class="list-group-item border-secondary py-3 text-white" style="background-color: rgba(255,255,255,0.02) !important;">
                        <div class="row align-items-center">
                            <div class="col-12 col-md-auto text-center mb-3 mb-md-0 d-flex justify-content-center align-items-center" style="width: 80px;">
                                <span class="badge border border-secondary p-2 d-inline-block text-center shadow-sm" style="background: rgba(255,255,255,0.05); min-width: 60px;">
                            ...
                            <div class="col-12 col-md mb-3 mb-md-0 text-center text-md-start">
                                <div class="fw-bold fs-6">SSID: {{ $interface['ssid'] ?? '?' }} <span class="fw-normal text-muted mx-1">|</span> Modo: <span class="fw-normal">{{ ucfirst($interface['mode'] ?? '?') }}</span></div>
                                <div class="text-soft small fw-bold">BSSID: <span class="fw-normal">{{ $interface['bssid'] ?? '98:BA:5F:C5:XX:XX' }}</span> <span class="text-muted mx-1">|</span> Encriptación: <span class="fw-normal">{{ $interface['encryption'] ?? '-' }}</span></div>
                            </div>
                            <div class="col-12 col-lg-auto d-flex gap-2 justify-content-center">
                                <button type="button" class="btn btn-secondary btn-sm fw-bold text-dark px-3" style="background: #e0e0e0; font-size:0.75rem;">DESACTIVAR</button>
                                <button type="button" class="btn btn-sm text-white fw-bold px-3 btnEditInterface" style="background: #3eb2cc; font-size:0.75rem;" data-bs-toggle="modal" data-bs-target="#editWifiModal"
                                    data-id="{{ $interface['id'] }}"
                                    data-ssid="{{ $interface['ssid'] ?? '' }}"
                                    data-mode="{{ $interface['mode'] ?? 'ap' }}"
                                    data-network="{{ $interface['network'] ?? 'lan' }}"
                                    data-hidden="{{ $interface['hidden'] ?? '0' }}"
                                    data-wmm="{{ $interface['wmm'] ?? '1' }}"
                                ...
                                <form action="{{ route('network.wifi.delete') }}" method="POST" class="m-0 p-0">
                                    @csrf
                                    <input type="hidden" name="interface_id" value="{{ $interface['id'] }}">
                                    <button type="submit" class="btn btn-danger btn-sm fw-bold px-3" style="background: #db4444; font-size:0.75rem;" onclick="return confirm('¿Seguro que deseas eliminar esta interfaz ({{ $interface['ssid'] ?? $interface['id'] }})? Se removerá la configuración de la red.')">ELIMINAR</button>
                                </form>
                            </div>
                        </div>
                    </div>
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content bg-dark text-white border-secondary">
            <div class="modal-header border-secondary">
                <h5 class="modal-title" id="editWifiModalLabel"><i class="bi bi-pencil-square me-2"></i>Editar Configuración Wi-Fi</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body p-0">
                <form action="{{ route('network.wifi.edit') }}" method="POST" id="formEditWifi">
                    @csrf
                    <input type="hidden" name="interface_id" id="editInterfaceName" value="">
                    
                    <!-- Tabs Nav -->
                    <ul class="nav nav-tabs border-secondary bg-secondary bg-opacity-25" id="editWifiTabs" role="tablist">
                        <li class="nav-item" role="presentation">
                            <button class="nav-link active text-white border-secondary border-bottom-0" id="edit-general-tab" data-bs-toggle="tab" data-bs-target="#edit-general-pane" type="button" role="tab" aria-controls="edit-general-pane" aria-selected="true" style="background-color: transparent;">Configuración general</button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button class="nav-link text-soft border-secondary border-bottom-0" id="edit-security-tab" data-bs-toggle="tab" data-bs-target="#edit-security-pane" type="button" role="tab" aria-controls="edit-security-pane" aria-selected="false" style="background-color: transparent;">Seguridad Wi-Fi</button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button class="nav-link text-soft border-secondary border-bottom-0" id="edit-mac-tab" data-bs-toggle="tab" data-bs-target="#edit-mac-pane" type="button" role="tab" aria-controls="edit-mac-pane" aria-selected="false" style="background-color: transparent;">Filtro por MAC</button>
                        </li>
                    </ul>
                    ...
                    <div class="tab-content custom-tabs-content p-4" id="editWifiTabsContent">
                        ...
                        <div class="tab-pane fade show active" id="edit-general-pane" role="tabpanel" aria-labelledby="edit-general-tab" tabindex="0">
                            <div class="row mb-3 align-items-center">
                                <label class="col-sm-3 col-form-label text-md-end text-soft">Modo</label>
                                <div class="col-sm-9">
                                    <select name="mode" id="editMode" class="form-select bg-dark text-white border-secondary">
                            ...
                            <div class="row mb-3 align-items-center">
                                ...
                                <div class="col-sm-9">
                                    <input type="text" name="ssid" id="editSsid" class="form-control bg-dark text-white border-secondary" required>
                                </div>
                            </div>
                            ...
                            <div class="row mb-3 align-items-center">
                                ...
                                <div class="col-sm-9">
                                    <select name="network" id="editNetwork" class="form-select bg-dark text-white border-secondary">
                            ...
                            <div class="row mb-3 align-items-center">
                                ...
                                <div class="col-sm-9">
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" name="hidden" id="editHiddenSSID">
                        ...
                        <div class="tab-pane fade" id="edit-security-pane" role="tabpanel" aria-labelledby="edit-security-tab" tabindex="0">
                            <div class="row mb-3 align-items-center">
                                <label class="col-sm-3 col-form-label text-md-end text-soft">Encriptación</label>
                                <div class="col-sm-9">
                                    <select name="encryption" id="editEncryption" class="form-select bg-dark text-white border-secondary">
                            ...
                            <div class="row mb-3 align-items-center d-none" id="editPasswordContainer">
                                ...
                                <div class="col-sm-9">
                                    <input type="text" name="password" id="editPassword" class="form-control bg-dark text-white border-secondary" minlength="8" disabled>
                                    <div class="form-text text-muted">La clave debe tener un mínimo de 8 caracteres. Nota: la contraseña se muestra en texto plano para tu conveniencia.</div>
                                </div>
                            </div>
                        </div>

                        <!-- Pestaña Filtro por MAC -->
                        <div class="tab-pane fade" id="edit-mac-pane" role="tabpanel" aria-labelledby="edit-mac-tab" tabindex="0">
                            <div class="row mb-3 align-items-center">
                                <label class="col-sm-3 col-form-label text-md-end text-soft">Filtro MAC</label>
                                <div class="col-sm-9">
                                    <select name="macfilter" id="editMacFilter" class="form-select bg-dark text-white border-secondary">
                            ...
                            <div class="row mb-3 align-items-start d-none" id="editMacListContainer">
                                ...
                                <div class="col-sm-9">
                                    <textarea name="maclist" id="editMacList" class="form-control bg-dark text-white border-secondary" rows="3" placeholder="Ej: 00:11:22:33:44:55&#10;AA:BB:CC:DD:EE:FF" disabled></textarea>
                                    <div class="form-text text-muted">Ingrese múltiples direcciones MAC separadas por salto de línea o coma.</div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="modal-footer border-secondary">
                        ...
                        <button type="submit" class="btn btn-primary fw-bold" style="background-color: #397cbd;">GUARDAR CONFIGURACIÓN</button>
                    </div>
                </form>
            </div>
        </div>
...
<div class="modal fade" id="scanWifiModal" tabindex="-1" aria-labelledby="scanWifiModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content bg-dark text-white border-secondary">
            <div class="modal-header border-secondary">
                <h5 class="modal-title" id="scanWifiModalLabel"><i class="bi bi-search me-2"></i>Redes WiFi disponibles</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body p-0">
                <div class="table-responsive">
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (formConnect) {
            formConnect.addEventListener('submit', function(e) {
                ...
                fetch('{{ route('network.wifi.connect') }}', {
                    method: 'POST',
                    headers: {
...
<div class="modal fade" id="connectWifiModal" tabindex="-1" aria-labelledby="connectWifiModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content bg-dark text-white border-secondary">
            <div class="modal-header border-secondary">
                <h5 class="modal-title" id="connectWifiModalLabel">Conectarse a: <span id="connectModalSsidStr" class="text-info"></span></h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form id="connectWifiForm">
                    ...
                    <div class="mb-3 form-check">
                        <input type="checkbox" class="form-check-input" id="replaceConfig" name="replace_config" checked disabled>
                        <label class="form-check-label text-soft" for="replaceConfig">Reemplazar la configuración Wi-Fi actual</label>
                    </div>

                    <div class="mb-3">
                        <label for="connectNetwork" class="form-label text-soft">Nombre de la nueva red lógica</label>
                        <input type="text" class="form-control bg-dark text-white border-secondary" id="connectNetwork" name="network" value="wwan" required>
                    </div>

                    <div class="mb-3">
                        <label for="connectPassword" class="form-label text-soft">Contraseña WPA</label>
                        <input type="password" class="form-control bg-dark text-white border-secondary" id="connectPassword" name="password" minlength="8" required>
                    </div>
...
<div class="modal fade" id="addWifiModal" tabindex="-1" aria-labelledby="addWifiModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content bg-dark text-white border-secondary">
            <div class="modal-header border-secondary">
                <h5 class="modal-title" id="addWifiModalLabel"><i class="bi bi-plus-circle me-2"></i>Añadir red Wi-Fi</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body p-0">
                <form action="{{ route('network.wifi.add') }}" method="POST" id="formAddWifi">
                    @csrf
                    <input type="hidden" name="device" id="addDeviceName" value="radio0">
                    
                    <!-- Tabs Nav -->
                    <ul class="nav nav-tabs border-secondary bg-secondary bg-opacity-25" id="addWifiTabs" role="tablist">
                        <li class="nav-item" role="presentation">
                            <button class="nav-link active text-white border-secondary border-bottom-0" id="general-tab" data-bs-toggle="tab" data-bs-target="#general-pane" type="button" role="tab" aria-controls="general-pane" aria-selected="true" style="background-color: transparent;">Configuración general</button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button class="nav-link text-soft border-secondary border-bottom-0" id="security-tab" data-bs-toggle="tab" data-bs-target="#security-pane" type="button" role="tab" aria-controls="security-pane" aria-selected="false" style="background-color: transparent;">Seguridad Wi-Fi</button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button class="nav-link text-soft border-secondary border-bottom-0" id="mac-tab" data-bs-toggle="tab" data-bs-target="#mac-pane" type="button" role="tab" aria-controls="mac-pane" aria-selected="false" style="background-color: transparent;">Filtro por MAC</button>
                        </li>
                    </ul>
                    ...
                    <div class="tab-content custom-tabs-content p-4" id="addWifiTabsContent">
                        ...
                        <div class="tab-pane fade show active" id="general-pane" role="tabpanel" aria-labelledby="general-tab" tabindex="0">
                            ...
                            <div class="row mb-3 align-items-center">
                                <label class="col-sm-3 col-form-label text-md-end text-soft">ESSID</label>
                                <div class="col-sm-9">
                                    <input type="text" name="ssid" class="form-control bg-dark text-white border-secondary" required placeholder="Ej: OpenWrt_Nuevo">
                                </div>
                            </div>
                            ...
                            <div class="row mb-3 align-items-center">
                                ...
                                <div class="col-sm-9">
                                    ...
                                    <div class="form-check mt-2">
                                        <input class="form-check-input" type="checkbox" name="wmm" id="activateWMM" checked>
                        ...
                        <div class="tab-pane fade" id="security-pane" role="tabpanel" aria-labelledby="security-tab" tabindex="0">
                            <div class="row mb-3 align-items-center">
                                <label class="col-sm-3 col-form-label text-md-end text-soft">Encriptación</label>
                                <div class="col-sm-9">
                                    <select name="encryption" id="addEncryption" class="form-select bg-dark text-white border-secondary">
                            ...
                            <div class="row mb-3 align-items-center d-none" id="addPasswordContainer">
                                <label class="col-sm-3 col-form-label text-md-end text-soft">Clave</label>
                                <div class="col-sm-9">
                                    <input type="password" name="password" id="addPassword" class="form-control bg-dark text-white border-secondary" minlength="8" disabled>
                                    <div class="form-text text-muted">La clave debe tener un mínimo de 8 caracteres.</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="tab-pane fade" id="mac-pane" role="tabpanel" aria-labelledby="mac-tab" tabindex="0">
                            <div class="row mb-3 align-items-center">
                                <label class="col-sm-3 col-form-label text-md-end text-soft">Filtro MAC</label>
                                <div class="col-sm-9">
                                    <select name="macfilter" id="addMacFilter" class="form-select bg-dark text-white border-secondary">
                            ...
                            <div class="row mb-3 align-items-start d-none" id="addMacListContainer">
                                <label class="col-sm-3 col-form-label text-md-end text-soft">Direcciones MAC</label>
                                <div class="col-sm-9">
                                    <textarea name="maclist" id="addMacList" class="form-control bg-dark text-white border-secondary" rows="3" placeholder="Ej: 00:11:22:33:44:55&#10;AA:BB:CC:DD:EE:FF" disabled></textarea>
                                    <div class="form-text text-muted">Ingrese múltiples direcciones MAC separadas por salto de línea o coma.</div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="modal-footer border-secondary">
                        <button type="button" class="btn btn-secondary fw-bold" data-bs-dismiss="modal">DESCARTAR</button>
                        <button type="submit" class="btn btn-primary fw-bold" style="background-color: #397cbd;">GUARDAR</button>
                    </div>
                </form>
            </div>
        </div>
